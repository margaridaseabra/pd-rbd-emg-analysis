...
MANIFEST = Path("/Users/margaridaseabra/Library/CloudStorage/OneDrive-UniversityofCopenhagen/PD-Katia/Data/prepared_data/manifests/EEG_only/PD_week2_week21_inference_finalWT_512hz_EEGonly_completed.csv")
if not MANIFEST.exists():
    MANIFEST = Path("/Users/margaridaseabra/Library/CloudStorage/OneDrive-UniversityofCopenhagen/PD-Katia/Data/prepared_data/manifests/EEG_only/PD_week2_week21_inference_finalWT_512hz_EEGonly.csv")

OUT_DIR = MANIFEST.parent / "PD_transfer_evaluation_EEGonly"
...
def normalize_state(s):
    ...
    mapping = {
        ...
        "Undefined": "Undefined", "undefined": "Undefined", "ND": "Undefined", "TR": "Undefined",
...
df["manual_path_chosen"] = df["file_path_manual_state_annotation"].map(choose_manual_annotation_path)
df["manual_exists"] = df["manual_path_chosen"].map(lambda p: p is not None and Path(p).exists())
...
print("PD rows for EEG-only evaluation:", len(pd_eval))
...
for _, row in pd_eval.iterrows():
    manual_path = Path(row["manual_path_chosen"])
...
segment_metrics.to_csv(OUT_DIR / "PD_EEGonly_segment_metrics.csv", index=False)
failed.to_csv(OUT_DIR / "PD_EEGonly_failed_rows.csv", index=False)
...
confusion = pd.crosstab(
    ...
    pd.Series(all_auto, name="EEG-only Somnotate")
).reindex(index=STATES, columns=STATES, fill_value=0)

confusion.to_csv(OUT_DIR / "PD_EEGonly_confusion_counts.csv")

confusion_row_pct = confusion.div(confusion.sum(axis=1), axis=0) * 100
confusion_row_pct.to_csv(OUT_DIR / "PD_EEGonly_confusion_row_percent.csv")
...
state_metrics.to_csv(OUT_DIR / "PD_EEGonly_state_metrics.csv", index=False)
...
week_summary.to_csv(OUT_DIR / "PD_EEGonly_week_summary.csv", index=False)
mouse_summary.to_csv(OUT_DIR / "PD_EEGonly_mouse_summary.csv", index=False)

# Accuracy by mouse/week
...
plt.ylim(0, 100)
plt.title("EEG-only WT model applied to PD: agreement with manual scoring")
plt.tight_layout()
plt.savefig(OUT_DIR / "PD_EEGonly_accuracy_by_mouse_week.png", dpi=180)
plt.close()
...
plt.xlabel("EEG-only Somnotate")
plt.ylabel("Manual")
plt.title("PD EEG-only confusion matrix (% of manual state)")
...
plt.colorbar(label="%")
plt.tight_layout()
plt.savefig(OUT_DIR / "PD_EEGonly_confusion_matrix.png", dpi=180)
plt.close()

# Manual vs auto REM occupancy
plt.figure(figsize=(9, 4.8))
...
plt.bar(x + width/2, segment_metrics["auto_pct_REM"].values, width, label="EEG-only")
plt.xticks(x, labels, rotation=60, ha="right")
plt.ylabel("% REM")
plt.title("PD EEG-only: manual vs model REM occupancy by segment")
plt.legend()
plt.tight_layout()
plt.savefig(OUT_DIR / "PD_EEGonly_REM_occupancy_manual_vs_auto.png", dpi=180)
plt.close()

print("\nPD EEG-only transfer evaluation complete.")
print("Output folder:", OUT_DIR)
print()
print("Overall PD EEG-only agreement accuracy:", f"{overall_accuracy:.2f}%")
