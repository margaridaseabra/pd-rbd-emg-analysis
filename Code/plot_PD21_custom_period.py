...
# Reuse functions/settings from your existing script.
# This assumes plot_PD21_interesting_REM_moments.py is in the same Code folder.
from plot_PD21_interesting_REM_moments import (
    CANDIDATE_MANIFESTS,
    OUT_DIR,
    EPOCH_SEC,
    FMAX,
...
CUSTOM_OUT_DIR = Path.home() / "Desktop" / "PD21_custom_period_plots"
...
def plot_custom_period(row, start_min, end_min, pad_min=2.0):
    ...
    if len(t_epoch_min) == 0:
        raise ValueError("Selected period is outside the available probability/state data.")
    ...
    title = (
        f"Custom PD week 21 period | mouse {row['mouse_id']} | segment {row['segment_id']} | "
        f"{start_min:.2f}–{end_min:.2f} min\n"
        f"Predicted states in selected period: {pred_summary} | "
    ...
    out = CUSTOM_OUT_DIR / (
        f"custom_PD21_mouse{row['mouse_id']}_seg{row['segment_id']}_"
...
def main():
    ...
    parser.add_argument("--list", action="store_true", help="List available PD week 21 rows and exit.")
    ...
    print("Available PD week 21 rows:")
    ...
    if len(matches) == 0:
        raise SystemExit("No matching PD week 21 recording found. Use --list to see available rows.")
